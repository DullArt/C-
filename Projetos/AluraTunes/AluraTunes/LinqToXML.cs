...
class LinqToXML
{
    ...
    {
        XElement root = new XElement(@"~\Data\AluraTunes.xml");
            var queryXML = 
            ...
            var query = from g in root.Element("Generos").Elements("Genero")
                        join m in root.Element("Musicas").Elements("Musica") on
                        g.Element("GeneroId").Value equals m.Element("GeneroId").Value
                        select new 
                        {
